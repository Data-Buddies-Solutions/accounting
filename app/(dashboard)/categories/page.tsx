      ...
      <Card>
        ...
        <CardContent>
          <div className="grid gap-3">
            {incomeCategories.filter(c => !c.parentId).map((category) => (
              <div key={category.id}>
                ...
                {/* Subcategories */}
                {incomeCategories.filter(c => c.parentId === category.id).map((subcat) => (
          ...
          <div className="grid gap-3">
            {expenseCategories.filter(c => !c.parentId).map((category) => (
              <div key={category.id}>
                ...
                {/* Subcategories */}
                {expenseCategories.filter(c => c.parentId === category.id).map((subcat) => (
                  <div key={subcat.id} className="flex items-center justify-between p-3 border-l-2 ml-8 mt-2 rounded-r-lg hover:bg-gray-50">
                    <div className="flex items-center gap-3">
                      <span className="text-xl">{subcat.icon}</span>
                      <div>
